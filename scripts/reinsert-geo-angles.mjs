...
// ── 1. O'chirish ──────────────────────────────────────────────────────────────
const ids = Array.from({length:91},(_,i)=>`geo-a-${String(i+1).padStart(3,'0')}`);
const {error:delErr} = await supabase.from('questions').delete().in('id', ids);
if (delErr) { console.error('O\'chirish xatosi:', delErr.message); process.exit(1); }
console.log('🗑  91 ta eski yozuv o\'chirildi');
...
// ── Renderers (AngleProblemsSite1-.jsx bilan bir xil) ─────────────────────────
...
// AdjBisector — α/β label (eski °daraja edi, yangi faylda α/β)
...
// Intersect — fixed small=50, labels array (yangi fayl)
function intersect(labels){
  const small=50,big=180-small,W=240,H=200,O=[W/2,H/2],R=88;
  const degs=[90-small/2,90+small/2,270-small/2,270+small/2];
  const ends=degs.map(d=>xy(O,d,R));
  const regions=[[degs[0],degs[1]],[degs[1],degs[2]],[degs[2],degs[3]],[degs[3],degs[0]+360]];
  const arcs=regions.map(([d1,d2],i)=>{const sa1=sa(d1),sa2=sa(d2);const[mx,my]=bp(O,40,sa1,sa2);return`<path d="${arc(O,26,sa1,sa2)}" fill="none" stroke="#2563eb" stroke-width="1.4"/><text x="${mx.toFixed(1)}" y="${my.toFixed(1)}" font-size="12.5" fill="#2563eb" text-anchor="middle">${labels[i]}</text>`;}).join('');
...
function makeSvg(d){
  ...
  switch(d.type){
    ...
    case'doublebis':return doubleBis(d.right,d.rl,d.ll);
    case'rightplus':return rightPlus(d.bottom,d.bl,d.tl);
    case'intersect':return intersect(d.labels);
    default:return null;
  }
}
...
// ── 91 masala (AngleProblemsSite1-.jsx bilan TO'LIQ bir xil) ─────────────────
const P=[
  ...
  {n:68,cat:3,q:"O'zaro qo'shni α va β burchaklarning bissektrisalari orasidagi burchakni toping.",a:'90°',d:{type:'doublebis',right:70,rl:'α',ll:'β'}},
  ...
  {n:81,cat:5,q:"Kesishgan ikki to'g'ri chiziqdan hosil bo'lgan burchaklardan biri 135°. α1, α2, α3 ni toping.",a:'α1 = 45°, α2 = 135°, α3 = 45°',d:{type:'intersect',labels:['135°','α3','α2','α1']}},
  {n:82,cat:5,q:"Kesishgan ikki to'g'ri chiziqdan hosil bo'lgan burchaklardan biri 60°. α + β yig'indisini toping.",a:'240°',d:{type:'intersect',labels:['60°','α','β','?']}},
  {n:83,cat:5,q:'(140° − α) va (2α − 40°) — qarama-qarshi (vertikal) burchaklar. α ni toping.',a:'60° (burchaklar — 80° va 100°)',d:{type:'intersect',labels:['(140°−α)','?','(2α−40°)','?']}},
  {n:84,cat:5,q:'sin α2 = √3/2, α2 > 90°. α1, α3, α4 ni toping.',a:'α1 = 60°, α3 = 60°, α4 = 120°',d:{type:'intersect',labels:['α1','α2','α3','α4']}},
  {n:85,cat:5,q:'tg α4 = −√3/3. α1, α2, α3 ni toping.',a:'α1 = 30°, α2 = 150°, α3 = 30°',d:{type:'intersect',labels:['α1','α2','α3','α4']}},
  {n:86,cat:5,q:"Kesishishdan hosil bo'lgan burchaklardan uchtasining yig'indisi 300° (α + 2β = 300°). α, β ni toping.",a:'60° va 120°',d:{type:'intersect',labels:['β','α','β','α']}},
  {n:87,cat:5,q:"O'tkir burchak va unga qo'shni ikki o'tmas burchakning yig'indisi 340° ga teng. Shu burchaklarni toping.",a:'20° va 160°',d:{type:'intersect',labels:['?','?','?','?']}},
  {n:88,cat:5,q:"O'tkir burchak va unga qo'shni ikki o'tmas burchakning yig'indisi 320° ga teng. Shu burchaklarni toping.",a:'40° va 140°',d:{type:'intersect',labels:['?','?','?','?']}},
  {n:89,cat:5,q:"O'tmas burchak va unga qo'shni ikki o'tkir burchakning yig'indisi 210° ga teng. Shu burchaklarni toping.",a:'150° va 30°',d:{type:'intersect',labels:['?','?','?','?']}},
  {n:90,cat:5,q:"Rasmda α va (α+65°) burchaklar ikki marta takrorlanadi. α ni toping.",a:'57,5°',d:{type:'intersect',labels:['α+65°','α','α+65°','α']}},
  {n:91,cat:5,q:"Kesishishdan hosil bo'lgan burchaklar o'zaro 2:3:2:3 nisbatda. Shu to'g'ri chiziqlar orasidagi o'tkir burchakni toping.",a:'72°',d:{type:'intersect',labels:['2k','3k','2k','3k']}},
];
...
console.log("🎉 Barcha 91 ta savol yangilandi!");
