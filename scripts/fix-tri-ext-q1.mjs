...
// 1. Delete old
const { error: delErr } = await supabase.from('questions').delete().eq('id', 'tri-ext-001');
...
// 2. Re-insert with correct topic (burchaklari with k)
const { error } = await supabase.from('questions').insert([{
  ...
  topic: "Uchburchakning tashqi burchaklari",
  difficulty: "O'rtacha",
  exam_type: ["DTM", "Milliy Sertifikat", "Maktab"],
  diagram_svg: svg,
}]);
if (error) { console.error('Kiritish xatosi:', error.message); process.exit(1); }
console.log("✅ tri-ext-001 to'g'ri mavzu bilan qayta kiritildi: 'Uchburchakning tashqi burchaklari'");
